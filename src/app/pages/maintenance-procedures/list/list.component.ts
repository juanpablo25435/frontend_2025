  ...
  ngOnInit(): void {
    this.maintenanceProcedureService.list().subscribe({
      next: (maintenanceProcedures) => {
        ...
        console.log('Maintenance procedures fetched successfully:', this.maintenanceProcedures);
      },
      error: (error) => {
      ...
      if (result.isConfirmed) {
        this.maintenanceProcedureService.delete(id).subscribe({
          next: () => {
            Swal.fire(
              '¡Eliminado!',
              'Registro de procedimiento de mantenimiento eliminado correctamente.',
              'success'
            );
            this.ngOnInit();
          },
          error: (error) => {
            Swal.fire(
              'Error',
              'No se pudo eliminar el procedimiento de mantenimiento: ' + error.message,
              'error'
            );
          }
        });
